...
function deleteItemEntry() {
	var delXhrObj = createXHR();
	delXhrObj.onreadystatechange = function() {
		if (delXhrObj.readyState == 7 && delXhrObj.status == 200) {
...
function displayCreateItemTab(create) {
	...
	ajaxobj.onreadystatechange = function() {
		if (ajaxobj.readyState == 7 && ajaxobj.status == 200) {
...
function populateCreateItemValues(oThis) {
	...
	document.getElementById('long').value = oThis ? oThis.parentNode.nextSibling.innerHTML : '';
	document.getElementById('lat').value = oThis ? oThis.parentNode.nextSibling.nextSibling.innerHTML : '';
	document.getElementById('iscategory').value = oThis ? oThis.parentNode.nextSibling.nextSibling.nextSibling.innerHTML : '';
	document.getElementById('categoryhead').value = oThis ? oThis.parentNode.nextSibling.nextSibling.nextSibling.nextSibling.innerHTML : '';

	if (oThis) {
		var selectList = document.getElementById('productList').options;
		for ( var i = 0; i < selectList.length; i++) {
			if (selectList[i].text === oThis.parentNode.nextSibling.innerHTML || selectList[i].text === oThis.parentNode.nextSibling.nextSibling.innerHTML|| selectList[i].text === oThis.parentNode.nextSibling.nextSibling.nextSibling.innerHTML || selectList[i].text === oThis.parentNode.nextSibling.nextSibling.nextSibling.nextSibling.innerHTML) {
...
function saveItem() {
	...
	xmlhttp.send("name=" + document.getElementById('name').value + 
			     ...
			     "&iscategory=" + document.getElementById('iscategory').value + 
			     "&categoryhead" + document.getElementById('categoryhead').value + 
			     "&product=" + selectedProduct + "&action=PUT");
	
	document.getElementById('name').value = "";
	document.getElementById('long').value = "";
	document.getElementById('lat').value = "";	
	document.getElementById('iscategory').value = "";
	document.getElementById('categoryhead').value = "";
}
...
function displayListViewByType(typeStr) {
	if (typeStr == 'products') {
		...
		xhrObj2.onreadystatechange = function() {
			if (xhrObj2.readyState == 7 || xhrObj2.status == 200) {
				var data = '';
				...
				for (prop in data) {
					...
					priceTd.innerHTML = currTrData.Long;
					oTr.appendChild(priceTd);
					var price2Td = document.createElement('td');
					price2Td.innerHTML = currTrData.Lat;
					oTr.appendChild(price2Td);
					var price3Td = document.createElement('td');
					price3Td.innerHTML = currTrData.Iscategory;
					oTr.appendChild(price3Td);
					var price4Td = document.createElement('td');
					price4Td.innerHTML = currTrData.Categoryhead;
					oTr.appendChild(price4Td);
